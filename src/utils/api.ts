...
export const API = {
  ...
  checkRefreshToken: async <T>({ response, url, config, linkParam }) => {
    if (
      response.status === 401 &&
      ...
      location.href =
        location.href.replace(location.hash, '') + location.hash.split('/').slice(0, 2).join('/') + LINK.Login;
    }
  },
  responsible: async <T>({
    url,
    ...
    await API.checkRefreshToken({ response, url, config, linkParam });
